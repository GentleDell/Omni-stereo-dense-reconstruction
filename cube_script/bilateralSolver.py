...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 18 21:39:14 2020
...
def bilateral_solver(target: np.array, cost: np.array, reference: np.array) -> np.array:  
    """
    Parameters
    ----------
    target : np.array
    ...
    cost : np.array
        Costs to evaluate the the confidence of eac pixel.

    reference : np.array
        A confidence image, representing how much we trust the values in "target". Pixels with zero confidence are ignored.
        Confidence can be set to all (2^16-1)'s to effectively disable it.
            
    Returns
        The filtered input image.
    -------
    None.
    ...
    output_solver = output_solver*max_target
